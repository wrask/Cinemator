<div class="relative mt-3 md:mt-0">
    <input wire:model.debounce.500ms="search" type="text" class="bg-gray-800 text-sm rounded-full w-64 px-4 py-1 pl-8 focus:shadow-outline" placeholder="Search">
    <div class="absolute top-0">
        <svg class="fill-current w-4 text-gray-500 mt-1 ml-2" viewBox="0 0 24 24" width="24" height="24">
    ...
    @if (strlen($search) >= 2)
        <div class="absolute bg-gray-800 text-sm rounded w-64 mt-4">
            ...
            @else
                <div class="px3 py-3">No results for {{ $search }}</div>
            @endif
        </div>
    @endif
</div>
